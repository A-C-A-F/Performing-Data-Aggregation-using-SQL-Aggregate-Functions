...
SELECT COUNT(*) AS no_of_records from salaries;
...
select COUNT(salary) AS sal_paid
from salaries;
...
SELECT DISTINCT(first_name) AS diff_first_names
FROM employees;
...
SELECT COUNT(DISTINCT(first_name)) AS diff_names_count
FROM employees;
...
SELECT COUNT(DISTINCT(dept_no)) AS dept_counts
FROM dept_emp;
...
--[Ariel Try]
SELECT COUNT(emp_no) AS emp_with_sal_80k_above
FROM salaries
WHERE salary > 80000;
...
-- Solution to 4.5
SELECT emp_no, COUNT(from_date) AS number_of_contracts
FROM dept_emp
...
SELECT ROUND(AVG(salary)) AS average_pay_rounded
FROM salaries;

-- 8.2: Round the average salary to a precision of cents.
SELECT ROUND(AVG(salary), 2) AS average_pay_rounded
FROM salaries;
